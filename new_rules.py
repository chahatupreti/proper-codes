...
#    print br3
    ...
    if re.search(r'(^!Series_.*?\s"[^\.]*?%s\s)' %gs1, s1, re.I|re.S): # GS in first line
#        print 33
        rg7.write(file)
        rg7.write('\n')
        rg7.write(gs11)
...
#        print 34
    ...
    if re.search(r'([\.] here(,)? we[^\.]*?%s)' %gs1, s1) : # starts with a fullstop b/c the 'Here we' line should be a new one. the (,)? means the comma is optional so it can match both 'here we' and 'here, we'. the [^\.] means any character other than fullstop, b/c we want the GS in the same sentence
#        print 34
        rg10.write(file)
        rg10.write('\n')
        rg10.write(gs11)
